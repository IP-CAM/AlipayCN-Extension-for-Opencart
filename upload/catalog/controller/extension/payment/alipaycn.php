...
require __DIR__ . '/../../../../system/library/alipaycn/php-library/autoload.php';
...
class ControllerExtensionPaymentAlipaycn extends Controller
{
    ...
    {
        $total_requirement = $this->config->get('payment_alipaycn_total') == null ? 1 : $this->config->get('payment_alipaycn_total');
        ...
        $token = $this->config->get('payment_alipaycn_token');
        $config = array(
            'token' => $token,
            'return_url' => $this->url->link('extension/payment/alipaycn/expressReturn', '', true),
        );
        ...
        $environment = $this->config->get('payment_alipaycn_test') == "sandbox" ?
        'staging' :
        'production';
        $redirect_url = '';
        ...
        try {
            ...
            switch (strtoupper($currency)) {
                ...
                case 'RMB':
                    ...
                    break;
                default:
                    die('Unsupported Currency for alipaycn');
            };
            ...
            $yedpay->setGateway(Client::INDEX_GATEWAY_ALIPAY_ONLINE)
                ...
                ->setReturnUrl($this->url->link('extension/payment/alipaycn/expressReturn', '', true))
                ->setNotifyUrl($this->url->link('extension/payment/alipaycn/expressNotify', '', true));
            $extraParam = json_encode([
                'out_trade_no' => $out_trade_no,
            ]);
            $response = $yedpay->precreate($this->config->get('payment_alipaycn_store_id'), $total_amount, $extraParam);
            if ($response instanceof Error) {
                $this->log->write('error: ' . $response->getMessage());
                die('Transaction Error. Please try again later' . $response->getErrors());
            }
...
//              $order->add_order_note(__("AliPay API payment failed. Couldn't connect to gateway server.", 'woocommerce'));
        ...
        $data['action'] = $redirect_url; //$gateway_url . 'precreate/' . $store_id;//$config['gateway_url'] . "?charset=" . $this->model_extension_payment_alipaycn->getPostCharset();
        $token_params = array();

        $token_params['return_url'] = $this->url->link('checkout/success', '', 'SSL');
        $data['form_params'] = $token_params; //$response;

        return $this->load->view('extension/payment/alipaycn', $data);
    }
    ...
    public function callback()
    {
        $result = false;
        $orderId = "";
        $this->log->write('alipaycn pay notify:');
        $arr = $_POST;
        $this->load->model('extension/payment/alipaycn');
        ...
        if ($result) { //check successed
            ...
            $this->load->model('checkout/order');
            $this->model_checkout_order->addOrderHistory($orderId, $this->config->get('payment_alipaycn_order_status_id'));
